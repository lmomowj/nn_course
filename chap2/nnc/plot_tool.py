...
# 绘制数据分布图+决策边界
def draw_classification_region(fig_dir,fig_name,data,c_data,**kwargs):
...
# 感知器训练过程
def draw_perceptron(num,X,w,func=None,**kwargs):    
    if func is None:
        func = 'linear'        
    ...
    try:
        ...
        if (func == 'linear'):        
            ...
            plt.title('单层感知器学习规则',fontsize=14,fontproperties=kaiti_font_title)
            # 绘训练样本散点图
            for x in X:
                if x[1] <= 0:
                    plt.scatter(x[0][0], x[0][1], marker='o',color=palette[5],s=40)
                else:
                    plt.scatter(x[0][0], x[0][1], marker='*',color=palette[10],s=60)
            # 绘 判别边界线
            plt.plot(
                num,
                [afunc(i) for i in num],
                linewidth=2.5,
                color=palette[0],
